...
class Config:
    ...
    batch_size = 32
    num_epochs = 1
    max_steps = 30000  # 总迭代步数
    # SGD bs=32
    # learning_rate = 0.0050465
    # Adam bs=16
    # learning_rate = 0.00044039
    #AdamW bs=32
    learning_rate = 0.00084439
...
def train_loop_parallel(config, model, criterion, train_loader, test_loader,wandb_run):
    """使用定点迭代和滑动窗口的并行训练循环"""
    # 如果只有一个GPU，退回到常规训练
    if config.device_count <= 1:
        train_loop_serial(config, model, criterion, train_loader, test_loader,wandb_run)
        return
    ...
    wandb_run.log({"final_test_accuracy": final_accuracy,
...
def train_loop_serial(config, model, criterion, train_loader, test_loader,wandb_run):
    """常规的串行训练循环"""
...
def train_with_config(config_dict):
    ...
    if config.training_mode.lower() =="parallel":
        train_loop_parallel(config, model,criterion,train_loader, test_loader,wandb_run)
    else:
        train_loop_serial(config,model,criterion,train_loader,test_loader,wandb_run)
